...
async function convertBufferToDataRoot({
  data,
}: {
  data: Buffer;
}): Promise<string> {
  ...
  const { id: dataRoot } = await buildLayers(leaves);

  return toB64Url(Buffer.from(dataRoot));
}

// TODO: convert readable to data root

export class WebDataRootVerifier implements DataVerifier {
